...
public class ItemClickListener implements AdapterView.OnItemClickListener {
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		if(block.get(position)==0||this.first_position==position)
			...
			List<Integer> path = block.isMatch(position, this.first_position);
			...
			if(!path.isEmpty()){
				...
				((GameActivity)view.getContext()).setPathPosition(p);
				
				block.removeCard(this.first_position);//두 위치를 0으로 변경
				block.removeCard(position);
				

				((FrameLayout)parent.getChildAt(position)).removeAllViews(); //두 위치의 이미지만 제거 setGridViewChange()사용하는 것보다 빠름.
